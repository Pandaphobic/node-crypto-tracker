...
const getCoinPrices = async () => {
  try {
    const res = await fetch(
      `https://api.coingecko.com/api/v3/simple/price?ids=${COINS_TO_GET}&vs_currencies=${VS_CURRENCY}&include_24hr_change=true&include_last_updated_at=true`
    );
...
async function main() {
  ...
  aaveHealthFactor(coinPrices, VS_CURRENCY);
}
// Init
main();
